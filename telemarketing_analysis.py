...
def main():
    ...
    image = Image.open("/Users/joicealves/Downloads/Material_de_apoio_M19_Cientista de Dados/img/Bank-Branding.jpg")
    ...
    if (data_file_1 is not None):
        ...
        if graph_type == 'Barras':
            fig, ax = plt.subplots(1, 2, figsize=(10, 5))
            sns.barplot(x=bank_raw_target_perc.index, y='proportion', data=bank_raw_target_perc, ax=ax[0], order=['no', 'yes'])
            ax[0].bar_label(ax[0].containers[0])
            ax[0].set_title('Dados brutos', fontweight="bold")
    
    # Gráfico 2: Dados filtrados
            sns.barplot(x=bank_target_perc.index, y='proportion', data=bank_target_perc, ax=ax[1], order=['no', 'yes'])
